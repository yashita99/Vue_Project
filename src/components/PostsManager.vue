         ...
         <b-col>
          <table class="table table-striped table-borderes='1px'" v-if="status">
            ...
            <tr v-for="item in filteredList" v-bind:key="item.id">
                ...
                <td>{{item.email}}</td>
                <td>{{item.doc}}</td>
                <td><b-btn v-on:click="userDetails(item.id, item.name, item.email, item.doc)">Edit</b-btn>
...
export default {
  ...
  methods: {
    ...
    userDetails (id, name, email, doc) {
      this.$router.push({name: 'Edit', params: { id, name, email, doc }})
    },
    submitData (e) {
